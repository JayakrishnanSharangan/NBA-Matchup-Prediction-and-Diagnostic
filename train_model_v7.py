...
def compute_player_strength(df_p):
    ...
    # Optimized: sort and head(8) is much faster than apply(nlargest)
    df_sorted = df.sort_values(by=['GAME_ID', 'TEAM_ABBREVIATION', 'player_strength'], ascending=[True, True, False])
    agg = df_sorted.groupby(['GAME_ID', 'TEAM_ABBREVIATION']).head(8)
    agg = agg.groupby(['GAME_ID', 'TEAM_ABBREVIATION'])['player_strength'].sum().reset_index()
