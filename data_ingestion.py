...
def fetch_and_cache_games():
    print("==========================================================")
    print("[INFO] NBA HISTORICAL DATA INGESTION PIPELINE")
    print("[INFO] Fetching Regular Season + Playoffs (2000-01 to 2025-26)")
    print("==========================================================")

    os.makedirs("data", exist_ok=True)
    cache_path = os.path.join("data", "nba_all_seasons_games.csv")
    ...
    for season in seasons:
        for season_type_label, season_type_short in season_types:
            call_idx += 1
            print(f"[{call_idx}/{total_calls}] Fetching {season} - {season_type_label}...")
            ...
            while retries > 0 and not success:
                try:
                    ...
                    if df.empty:
                        print(f"  [WARN] No data for {season} {season_type_label}. Skipping.")
                    else:
                        df["SEASON"] = season
                    ...
                    retries -= 1
                    print(f"  [ERROR] Failed: {e}. Retries remaining: {retries}")
                    time.sleep(3)
    ...
    print(f"     Regular Season rows : {len(rs_count)}")
    print(f"     Playoffs rows       : {len(po_count)}")
    print(f"     Total rows saved    : {len(combined_df)}")
    print(f"     Saved to            : {cache_path}")
    print("==========================================================")

if __name__ == "__main__":
    fetch_and_cache_games()
